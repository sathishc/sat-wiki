---
title: Compounding
tags: [wealth, knowledge, relationships, long-term, naval]
created: 2026-04-16
updated: 2026-04-16
sources: [sources/naval-almanack.md]
layout: default
parent: Concepts
---

# Compounding

Compounding applies far beyond finance. [Naval Ravikant](../people/naval-ravikant.md) treats it as a universal law that rewards long-term thinking in all domains.

## Domains Where It Applies

| Domain | What Compounds |
|---|---|
| **Wealth** | Capital → returns → more capital |
| **Knowledge** | Understanding → better questions → deeper understanding |
| **Reputation** | Trust built over time → more opportunities → more trust |
| **Relationships** | History + goodwill → deeper collaboration → better outcomes |
| **Skills** | Practice → capability → ability to practise at higher levels |

## The Key Insight

Short-term thinking destroys compounding. Switching strategies, burning relationships, or cashing out early all forfeit the exponential tail.

> "Play long-term games with long-term people."
> — [Naval Ravikant](../people/naval-ravikant.md)

## Practical Implications

- Choose industries and problems where the knowledge is durable, not quickly obsolete
- Be consistent and reliable — reputation compounds through repeated interaction
- Avoid zero-sum games — they have no compounding because gains require losses
- The best investments (of time or money) often look boring for a long time, then suddenly obvious

## Related

- [Specific Knowledge](./specific-knowledge.md)
- [Leverage](./leverage.md)
- [Naval Ravikant](../people/naval-ravikant.md)
- [Lindy Effect](./lindy-effect.md) — long survival as signal of compounding quality
- [Deep Work](./deep-work.md) — the practice that makes knowledge compound faster
- [Amazon Growth Flywheel](./amazon-growth-flywheel.md) — compounding operationalised at corporate scale
- [Flow Economics](./flow-economics.md) — economic value compounds when flow is sustained

## Connections

- **[Naval Ravikant](../people/naval-ravikant.md)** — compounding is central to his wealth-building framework
- **[Specific Knowledge](./specific-knowledge.md)** — specific knowledge compounds over time into irreplaceable expertise
- **[Leverage](./leverage.md)** — leverage multiplies the rate of compounding
- **[Deep Work](./deep-work.md)** — deep work is the practice that makes knowledge compound faster
- **[Amazon Growth Flywheel](./amazon-growth-flywheel.md)** — compounding operationalised at corporate scale via a reinforcing loop
- **[Lindy Effect](./lindy-effect.md)** — long survival is evidence of compounding quality and durability
